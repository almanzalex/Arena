...
def _interaction(spec: dict[str, Any]) -> str:
    from rlx.plugins.interactions import require_interaction_kind
...
def _require_pz() -> None:
    try:
        ...
        raise RlxError(
            ...
            "Install with: pip install 'rlx[pettingzoo]'"
        ) from e
...
def _make_env_pettingzoo(spec: dict[str, Any]):
    ...
    ``rlx check`` therefore reflect the *wrapped* observation contract — never
    ...
    if env_id in _VECTOR_PILOT_ALIASES:
        ...
        if interaction in {"aec", "dynamic_aec"}:
            ...
            from rlx.adapters.task_pettingzoo.pilot_env import parallel_env
...
def _load_aec_env(env_id: str, config: dict[str, Any]):
    ...
    try:
        ...
        raise RlxError(f"cannot load AEC env {env_id!r}: {e}") from e
    if not hasattr(mod, "env"):
        raise RlxError(f"{module_path} does not expose env() for AEC")
    try:
...
def _load_parallel_env(env_id: str, config: dict[str, Any]):
    ...
    if family == "mpe" or env_id.startswith("mpe/"):
        raise RlxError(
            ...
            f"import. Install with: pip install mpe2  (or pip install 'rlx[pettingzoo]'). "
