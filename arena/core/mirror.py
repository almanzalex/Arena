"""Content-preserving artifact mirroring for file:// and credentialed stores.

Hugging Face ``hf://`` lives in ``arena.core.store_hf``.
OCI / W&B / MLflow live in ``arena.core.store_{oci,wandb,mlflow}``.
"""
...
def __getattr__(name: str) -> Any:
    """Lazy re-exports for extracted store adapters (avoid import cycles)."""
    if name == "HuggingFaceStoreAdapter":
        from arena.core.store_hf import HuggingFaceStoreAdapter

        return HuggingFaceStoreAdapter
