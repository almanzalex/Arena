...
from arena.core.errors import CompatibilityReport, SchemaError
...
class Task:
    ...
    def load(cls, ref: str | Path | dict[str, Any]) -> Task:
        ...
        if path.exists():
            ...
            if manifest.get("schema") == "arena.task/v0alpha1":
                validate_task_manifest(manifest)
            return cls(manifest)
        # Inline adapter ref string: pettingzoo://arena/competitive_rps_v0
...
class Population:
    """Content-addressed set of policy digests (Arena 0.2)."""
...
class Evaluation:
    """Versioned evaluation suite (Arena 0.2)."""
    ...
    def load(cls, path: str | Path) -> Evaluation:
        from arena.runtime.evaluation import load_evaluation

        return cls(load_evaluation(path))

    def validate(self, *, populations: dict[str, dict[str, Any]] | None = None) -> dict[str, Any]:
        from arena.runtime.evaluation import validate_evaluation
