"""Discriminated action-type cases for portable RLX policies.
...
Unknown kinds fail loud with an extension recipe (see ``rlx.core.registry``).
"""
...
def multidiscrete_slices(action: dict[str, Any]) -> list[tuple[int, int]]:
    ...
    if not isinstance(layout, dict):
        raise SchemaError(
            ...
            "RLX will not flatten MultiDiscrete to Discrete."
        )
...
def box_distribution(action: dict[str, Any]) -> str:
    from rlx.core.registry import DISTRIBUTIONS, ensure_plugins_loaded
...
def dict_key_order(action: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(order, list) or not order:
        raise SchemaError(
            ...
            "Unstable key order is rejected (RLX will not invent a sort)."
        )
...
def dict_param_slices(action: dict[str, Any]) -> list[tuple[str, str, int, int]]:
    ...
    if not isinstance(layout, dict) or layout.get("kind") != "concatenated_fields":
        raise SchemaError(
            ...
            "RLX will not coerce Dict↔vector."
        )
