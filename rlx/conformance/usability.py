...
def read_clean_room_guide(path: Path | str) -> list[str]:
    ...
    blocks = [
        ...
        if "rlx-clean-room" in match.group("info").split()
    ]
    if len(blocks) != 1:
        raise AssertionError("guide must contain exactly one rlx-clean-room command block")
    commands = [
        line.strip()
        for line in blocks[0].splitlines()
        if line.strip() and not line.lstrip().startswith("#")
    ]
    if not commands or any(not command.startswith("rlx ") for command in commands):
        raise AssertionError("guide must give a non-empty, rlx-only received-artifact flow")
    return commands
    ...
    *,
    ...
    for command in read_clean_room_guide(guide):
        ...
        if proc.returncode:
            ...
            break
    report = {
        "schema": "rlx.blind-reader-usability/v1",
        "started_at": started,
        "finished_at": _now(),
