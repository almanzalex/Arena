...
from arena.adapters.eval_gimitest import GimitestEvalProvider
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 2:
        raise SystemExit("usage: python -m arena.adapters.eval_gimitest.worker REQUEST RESPONSE")
    request_path, response_path = map(Path, args)
    request = load_manifest(request_path, max_bytes=16 * 1024 * 1024)
    if request.get("schema") != "arena.eval-provider-request/v1":
    ...
    response = {
        "schema": "arena.eval-provider-response/v1",
        "ok": True,
