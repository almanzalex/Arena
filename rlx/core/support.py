...
SUPPORT_SCHEMA = "rlx.support-matrix/v1"


def load_support_matrix() -> dict[str, Any]:
    payload = resources.files("rlx").joinpath("support-matrix.json").read_text(
        encoding="utf-8"
    )
    data = json.loads(payload)
    if data.get("schema") != SUPPORT_SCHEMA:
        raise SchemaError("installed RLX support matrix has an unsupported schema")
    return data


def load_schema_registry() -> dict[str, Any]:
    payload = resources.files("rlx").joinpath("schema-registry.json").read_text(
        encoding="utf-8"
    )
    data = json.loads(payload)
    if data.get("schema") != "rlx.schema-registry/v1":
        raise SchemaError("installed RLX schema registry has an unsupported schema")
    schemas = data.get("schemas")
    if not isinstance(schemas, list) or not schemas:
        raise SchemaError("installed RLX schema registry is empty")
    ids = [item.get("id") for item in schemas if isinstance(item, dict)]
    if len(ids) != len(schemas) or len(set(ids)) != len(ids):
        raise SchemaError("installed RLX schema registry has duplicate or invalid ids")
    return data
...
def _probe_isolated_python(
    ...
    *,
    ...
    version_match = versions.get("rlx") == release
    return {
        "status": "ready" if not missing and version_match else "incompatible",
        "versions": versions,
        "missing": missing,
        "rlx_version_match": version_match,
    }
...
def capability_report(name: str, *, matrix: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if name not in capabilities:
        raise SchemaError(
            ...
            repair="Run `rlx doctor` to list installed and supported capabilities.",
        )
    ...
    if not ready:
        ...
        if platform_state != "stable":
            repair = (
                f"RLX has no stable 1.0 claim for {current['os']}/{current['arch']} "
