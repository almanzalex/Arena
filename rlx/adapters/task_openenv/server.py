...
Run with ``python -m rlx.adapters.task_openenv.server --port 8000``. The server
uses OpenEnv's own FastAPI/WebSocket transport and wraps RLX's existing frozen
...
def create_app(env_kind: str = "rps") -> Any:
    try:
        ...
        raise RuntimeError("install the OpenEnv pilot with: pip install 'rlx[openenv]'") from e

    if env_kind == "vector":
        from rlx.adapters.task_pettingzoo.pilot_env import VectorCoordinationParallel
        ...
        return openenv_create_app(
            ...
            VectorAction,
            VectorObservation,
            env_name="rlx-vector-coordination-v0",
    ...
    return openenv_create_app(
        ...
        RPSAction,
        RPSObservation,
        env_name="rlx-competitive-rps-v0",
        max_concurrent_envs=16,
    )
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit("install the OpenEnv pilot with: pip install 'rlx[openenv]'") from e
