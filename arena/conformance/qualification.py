"""Reusable release qualification for an Arena adapter fixture.
...
Evaluation fixtures (``arena.evaluation/v0alpha1``) exercise population + eval gates.
"""
...
def qualify_evaluation_fixture(
    ...
    *,
    ...
    Expects an ``arena.evaluation/v0alpha1`` YAML whose assignments reference
    local population YAMLs / policy bundle paths.
    """
    from arena.core.population import create_population_from_yaml, load_population
    from arena.core.sdk import Policy
    from arena.core.store import LocalStore
    from arena.runtime.evaluation import build_eval_report, load_evaluation, run_evaluation
    ...
    with tempfile.TemporaryDirectory(prefix="arena-qualify-eval-") as raw:
        work = Path(raw)
        store_root = work / "ws"
        store_root.mkdir()
...
def qualify_adapter_fixture(
    ...
    *,
    ...
    ``fixture`` is an ordinary Arena match manifest **or** an evaluation suite
    (``arena.evaluation/v0alpha1``). Match fixtures keep the 0.1 gates; evaluation
    ...
    started = _utc_now()
    checks: dict[str, dict[str, Any]] = {}
    with tempfile.TemporaryDirectory(prefix="arena-qualify-") as raw:
        work = Path(raw)
        ...
        def run_once(name: str) -> Path:
            out = work / name
            proc = subprocess.run(
                [sys.executable, "-m", "arena", "match", "run", str(fixture), "--out", str(out)],
                text=True,
                capture_output=True,
                check=False,
            )
        ...
        malformed = work / "malformed.arena"
