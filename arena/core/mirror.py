...
# Thin re-exports: keep `from arena.core.mirror import HuggingFaceStoreAdapter` working
# while allowing direct imports of arena.adapters.store_* without circular imports.
_STORE_REEXPORTS = {
    "HuggingFaceStoreAdapter": ("arena.adapters.store_hf", "HuggingFaceStoreAdapter"),
    "OCIStoreAdapter": ("arena.adapters.store_oci", "OCIStoreAdapter"),
    "WandBStoreAdapter": ("arena.adapters.store_wandb", "WandBStoreAdapter"),
    "MLflowStoreAdapter": ("arena.adapters.store_mlflow", "MLflowStoreAdapter"),
    "_parse_hf_uri": ("arena.adapters.store_hf", "_parse_hf_uri"),
}


def __getattr__(name: str) -> Any:
    if name in _STORE_REEXPORTS:
        import importlib

        module_name, attr = _STORE_REEXPORTS[name]
...
def __dir__() -> list[str]:
    return sorted(set(globals()) | set(_STORE_REEXPORTS))
