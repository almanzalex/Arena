...
#
# Builds a minimal environment that contains ONLY the published RLX wheel (plus
# its declared extras) — no source repo, no trainer, no checkpoints. The build
# context must contain exactly one `rlx-*.whl`. The clean-room commands are then
# run against this image with `--network none`, so any hidden download fails.
#
# Build:  docker build -f docker/clean-room.Dockerfile -t rlx-cleanroom <context-with-wheel>
# Run:    docker run --rm --network none -v <sandbox>:/work -w /work rlx-cleanroom bash run_cleanroom.sh
