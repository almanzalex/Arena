...
This guide is for a second lab member who **did not author the trainer**. Goal: load an exported RLX policy, run a seeded match, and inspect trajectories **without** the original training repository.
...
- Before isolating the run, install the received wheel and its declared extras:
  `python -m pip install './rlx-0.2.0-py3-none-any.whl[torch,pettingzoo]'`.
...
1. Policy bundle directories (e.g. `player_0.rlx/`, `player_1.rlx/`) containing `policy.yaml` + `payloads/`
...
or arbitrary Python source from Researcher A. RLX 0.1 receives only fixed
...
`rlx-clean-room` so the automated gate can parse and execute *exactly* these
...
holds the received `player_0.rlx`, `player_1.rlx`, and `match.yaml`.
...
- `rlx check` exits 0 for each assigned role
- `rlx match run` completes requested episodes (`failure_count == 0` unless intentional)
...
| ImportError for torch/pettingzoo | Extras not installed | `pip install 'rlx[all]'` |
...
parses the fenced block whose info string is `bash rlx-clean-room`, strips
...
- the commands drift from the canonical U-01 flow (wrong command, wrong order,
  wrong arguments);
- any command shells out to something other than `rlx` (e.g. re-installs, calls
...
| Layer | Coverage |
|-------|----------|
| `tests/acceptance/test_u01_clean_room.py` | Fast in-process gate. Drives the real CLI end-to-end: `rlx policy export`/`verify` from checkpoints, then—after copying only the `.rlx` bundles into a fresh directory and deleting the trainer, checkpoints, and workspace (trainer off `PYTHONPATH`)—`rlx init`/`inspect`/`check`/`match run --record`/`data inspect` from bundles alone. Uses the developer's editable install. |
| `tests/acceptance/test_u01_hermetic.py` (`slow`) | Hermetic "never-trained machine" gate. Builds a real `wheel` (`python -m build`), creates a throwaway `python3.12 -m venv` with a scrubbed `HOME`/`XDG`/`PYTHONPATH`, installs **only the wheel** (`rlx[torch,pettingzoo]`) the way a stranger would (no editable repo path), copies **only** the `.rlx` bundles + `match.yaml` + this doc into the sandbox, asserts the trainer package/checkpoints/spec/source repo are absent and unimportable, disables the network for the run, and then executes the commands **parsed out of this document** in order. |
...
| "Installs RLX like a second person" | Installs the built **wheel** (not `pip install -e .`), resolving `[torch,pettingzoo]` extras, into an empty environment | Whether public PyPI actually serves the pinned deps on release day |
| "From the received bundles alone" | Only `*.rlx` + `match.yaml` + docs copied in; absence of trainer/spec/repo asserted | — |
...
- **Inputs:** the same hermetic sandbox contents (`*.rlx`, `match.yaml`,
...
- **Harness:** an agent loop with a restricted toolset — read files, run shell
  ...
  model no RLX-specific priors beyond the docs.
- **Success signal:** the agent, using its own reading of the prose (not a
  hard-coded command list), reaches the same success criteria — `rlx check`
  exits 0 per role, `rlx match run` reports `failures=0`, and a trajectory
