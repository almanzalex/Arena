"""Small, reproducible training recipes over RLX trajectory datasets."""
...
from rlx.core.dataset import dataset_content_digest
...
from rlx.core.manifests import dump_json, dump_yaml, load_manifest, validate_dataset_manifest
from rlx.core.sdk import Policy

TRAIN_RECIPE_SCHEMA = "rlx.train/v1"
TRAIN_RUN_SCHEMA = "rlx.train-run/v1"
TRAIN_CHECKPOINT_SCHEMA = "rlx.train-checkpoint/v1"
...
def _require_torch() -> Any:
    try:
        ...
        raise RlxError("rlx train requires PyTorch; install with: pip install 'rlx[torch]'") from exc
    return torch
...
def validate_train_recipe(recipe: dict[str, Any]) -> dict[str, Any]:
    ...
    from rlx.core.registry import TRAINERS, ensure_plugins_loaded
    ...
    *,
    ...
    try:
        ...
        run = {
            ...
            "output_policy": {"path": "policy.rlx", "digest": policy.digest},
            "checkpoint": {
        ...
        return {**run, "out": str(out_path), "policy": str(out_path / "policy.rlx")}
    except Exception:
