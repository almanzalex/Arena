...
from rlx.runtime.evaluation import _execute_evaluation_cell


def main(argv: list[str] | None = None) -> int:
    args = list(sys.argv[1:] if argv is None else argv)
    if len(args) != 2:
        raise SystemExit("usage: python -m rlx.runtime.eval_worker REQUEST RESPONSE")
    request_path, response_path = map(Path, args)
    request = load_manifest(request_path, max_bytes=64 * 1024 * 1024)
    if request.get("schema") != "rlx.eval-cell-request/v1":
    ...
    response = {
        "schema": "rlx.eval-cell-response/v1",
        "ok": True,
