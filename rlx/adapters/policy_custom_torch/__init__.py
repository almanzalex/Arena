...
from rlx.adapters.policy_custom_torch.preprocess import PreprocessPipeline, canonical_pipeline
...
from rlx.core.contracts import (
...
def _require_torch():
    try:
        ...
        raise RlxError(
            ...
            "Install with: pip install 'rlx[torch]'"
        ) from e
    return torch, nn, F
...
class TorchPolicyRuntime:
    """Executable policy loaded from an RLX bundle (no training-repo imports)."""
        ...
        try:
            ...
            if not allow_trace:
                raise SchemaError(
                    ...
                    "runtime tiers in this release, so RLX will not silently ship "
...
def generate_reference_cases(
    ...
    *,
    ...
    """Capture reference cases for later ``rlx policy verify``.
    ...
    *,
    ...
    if provenance != PROVENANCE_SOURCE:
        result["warning"] = (
            ...
            "(default for `rlx policy export`) or pass --source-test cases from the "
