"""Detached Ed25519 attestations for RLX artifact identity.

Signatures prove possession of an explicitly trusted key. RLX deliberately does
...
ATTESTATION_SCHEMA = "rlx.attestation/v1"
...
def _crypto() -> tuple[Any, Any, Any, Any]:
    try:
        ...
        raise RlxError(
            "RLX signing requires cryptography; install with: pip install 'rlx[signing]'"
        ) from exc
...
def _statement(
    *,
    ...
    return {
        ...
        "predicate": {
            "type": "rlx.artifact-authenticity/v1",
            "issuer": issuer,
            "key_id": key_id,
...
def verify_artifact_attestation(
    ...
    *,
    ...
    return {
        "schema": "rlx.attestation-verification/v1",
        "ok": True,
        "identity": artifact.identity,
        "kind": artifact.kind,
