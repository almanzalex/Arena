...
class RlxError(Exception):
    """Base error for expected RLX failures.
...
class ExternalUnavailableError(RlxError):
...
class IncompleteExecutionError(RlxError):
...
def diagnostic_from_exception(
    ...
    *,
    ...
    expected = isinstance(exc, RlxError)
    ...
    default_repairs = {
        ...
        "schema_compatibility": (
            "Correct the reported field or use a schema/version listed by `rlx doctor`."
        ),
        "integrity_authenticity": (
            "Do not use the artifact; obtain trusted bytes or the correct independent key."
        ),
        "external": (
            "Run `rlx doctor --capability NAME`, repair availability, and retry safely."
        ),
        "incomplete_cancelled": (
            "Inspect the incomplete attempt ledger, then retry the whole operation."
        ),
        "internal": (
            "Re-run with --debug and report the redacted traceback with the command and RLX version."
        ),
    }
    repair = (
    ...
    result: dict[str, Any] = {
        ...
        "docs_url": f"https://github.com/rlx-project/rlx/blob/main/docs/errors.md#{str(code).lower()}",
...
def exit_code_for_exception(exc: BaseException) -> int:
    return exc.exit_code if isinstance(exc, RlxError) else 70
